...
def run_game(board, ap):
    ...
    while(True):
        for unit in ordered_units(units):
            #If the unit is dead, don't take a turn
            if(unit.hp <= 0):
                continue
            ...
            #Move
            if(adjacent_target == None):
                move(unit, targets, board)

                #We need to see if we are now next to an adjacent target after the move
                adjacent_target = find_adjacent_target(unit, targets)
...
def find_targets(hero, units):
    return [unit for unit in units if unit.elf != hero.elf and unit.hp > 0]
